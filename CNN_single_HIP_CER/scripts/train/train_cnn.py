#!/usr/bin/env python3
"""Train the published HIP/CER one-dimensional CNN."""
...
from hip_cer_pipeline.paths import GENERATED_MODELS_DIR, GENERATED_RESULTS_DIR
...
def main() -> None:
    ...
    train_loader = DataLoader(
        ArrayDataset(X_train, y_train), batch_size=batch_size, shuffle=True
    )
    test_loader = DataLoader(ArrayDataset(X_test, y_test), batch_size=batch_size)

    counts = np.unique(y_train, return_counts=True)[1]
    class_weights = torch.as_tensor(1.0 / counts, dtype=torch.float32, device=device)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    dropout = 0.0
    learning_rate = 0.001
    weight_decay = 0.001
    epochs = 1 if args.smoke_test else args.epochs
    model = Simple1DCNN(2463, 2, dropout).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_number, (inputs, labels) in enumerate(train_loader):
            ...
            if args.smoke_test and batch_number == 0:
                break
        test_loss, accuracy, precision, recall, f1, auc = evaluate(
            model, test_loader, criterion, device
        )
        rows.append(
            [epoch + 1, running_loss / (batch_number + 1), test_loss, accuracy, precision, recall, f1, auc]
        )
    ...
    log_path = GENERATED_RESULTS_DIR / (
        f"training_log_b{batch_size}_do{dropout}_lr{learning_rate}_wd{weight_decay}.csv"
    )
    pd.DataFrame(
        rows,
        columns=["Epoch", "Train Loss", "Test Loss", "Test Accuracy", "Precision", "Recall", "F1", "AUC"],
    ).to_csv(log_path, index=False)
    model_path = GENERATED_MODELS_DIR / (
        f"Simple1DCNN_{learning_rate}_{batch_size}_{epochs}_{dropout:.2f}_seed{seed}_acc{accuracy:.4f}.pt"
    )
