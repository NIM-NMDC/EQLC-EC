"""PyTorch dataset and model definitions used by the CHD scripts."""
...
class ArrayDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        sample = torch.tensor(self.data[index], dtype=torch.float32).squeeze()
        label = torch.tensor(self.labels[index], dtype=torch.long)
        return sample, label


class Simple1DCNN(nn.Module):
    """The exact architecture stored in the published full-object checkpoints."""
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        outputs = outputs.view(-1, 64 * self.pool2_out_size)
